...
TreeNodePtr createTreeWithLevelOrder(int *data, int size) {
    ...
    TreeNodePtr p , head ;
    //创建并初始化根结点
    head = createTreeNode(data[i], NULL, NULL) ;
    p = head ;
    //创建并初始化队列
    QueuePtr queue = InitQueue() ;
    //根结点入队
    EnQueue(queue, p) ;
    //队列非空时，队首出队并访问其左右子树，将其左右子树根结点依次入队
    i++ ;
    while( !QueueEmpty(queue) && (i < size) )
    {
        p = GetHead(queue) ;
        DeQueue(queue) ;
        ...
        //创建结点
        TreeNodePtr pl = createTreeNode(data[i], NULL, NULL) ;
        //结点非空时，让p的左子树指向它
        if( (i<size)&&(data[i] != -1) )
        {
            p->left = pl ;
        ...
        //创建结点
        TreeNodePtr pr = createTreeNode(data[i], NULL, NULL) ;
        //结点非空时，让p的右子树指向它
        if( (i<size)&&(data[i] != -1) )
        {
            p->right = pr ;
...
int maxPathSum(TreeNodePtr root, int sum) {
    ...
    else
    {
        //加上当前结点的value
        sum += root->val ;
        //递归：当前结点的最大路径和等于它之前的路径和加上左子树/右子树路径和中的最大值
        return ( sum + max(maxPathSum(root->left, 0), maxPathSum(root->right, 0)) ) ;
    }
}

/** ToDO: 任务三：请你通过递归求取该二叉树的所有左子叶权重之和 */
int sumOfLeftLeaves(TreeNodePtr root) {
    //根结点为空时，左子叶权重之和为0，返回0
    if( root == NULL )
        return 0 ;
    //存在左子叶时
    if( ( root->left != NULL )&&( root->left->left == NULL )&&( root->left->right == NULL ) )
        //返回该结点左子叶的权重加上该结点右子树中所有左子叶权重之和
        return ( root->left->val + sumOfLeftLeaves(root->right));
    //最后返回根结点左子树所有左子叶权重之和加上根结点右子树所有左子叶权重之和
    return ( sumOfLeftLeaves(root->left) + sumOfLeftLeaves(root->right) ) ;
}

/** TODO: 任务四：请你通过递归求取该树的镜像，即翻转该二叉树 */
TreeNodePtr invertTree(TreeNodePtr root) {
    //当树为空时，翻转后的二叉树也为空，返回NULL
    if( root == NULL )
        return NULL ;
    //翻转左子树
